...
K = 5 400 is the only strike where microstructure noise is *below* the residual std — cleanest signal.  
...
- **`b` and `c` extrapolations: do not trust.** R² < 0.02 means the regression line is noise. Projecting B and C linearly gives B_LIVE = 0.028 (12× the OLD B) and C_LIVE = −0.19 (sign flip from OLD +0.57) — these are extrapolation artefacts, not real term structure. **Keep OLD B and C.** Verified empirically below: per-strike bias is *worse* under LIVE projection than under OLD coefficients.
...
LIVE projection makes K=5000, 5100, 5400 *worse* — confirms B/C extrapolation is unreliable. **Use OLD A,B,C with per-strike bias correction.**
...
*Generated from Round 3 analysis — scripts in `Round_3/` and `Round_3/garch/`.*
